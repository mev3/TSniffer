        ...
        if args.graph:
            ...
            try:
                ...
                if args.outform == "json":
                    solidity_result_path = os.getcwd() + "//solidity_result//"
                    if not os.path.exists(solidity_result_path):
                        os.makedirs(solidity_result_path)
                    input_contract_address = ""
                    if len(args.solidity_files)!= 0:
                        input_contract_address = re.split("/", args.solidity_files[0])[-1].split(".")[0]
                    else:
                        input_contract_address = re.split("/", args.codefile.name)[-1].split(".")[0]
                    solidity_result_name = solidity_result_path + input_contract_address + ".json"
                    solidity_result_file = open(solidity_result_name, 'w')
                    solidity_result_file.write(outputs["json"])
                    solidity_result_file.close()
