...
# from mythril.analysis.module.modules.fake_token import OwnerBalanceAnnotation, BalanceAnnotation
...
class Instruction:
    ...
    def sha3_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if len(data_list) > 1:
            ...
            return [global_state]
        # for annotation in global_state.world_state.annotations:
        #     if isinstance(annotation, BalanceAnnotation) and int(data_list[-1]) == annotation.slot:
        #         is_balance = True
        result = keccak_function_manager.create_keccak(data)
        state.stack.append(result)
        # if is_balance and not(result.symbolic):
        #     state.stack[-1].annotate(OwnerBalanceAnnotation(int(data_list[-1])))
        return [global_state]
    ...
    def sload_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        # source_code = global_state.environment.contract.get_source_info(global_state.instruction["address"], False).code
        # print("sload source_code:")
        # print(source_code)
        return [global_state]
    ...
    def jumpi_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if negated_cond:
            ...
            log.debug("Pruned unreachable states jumpi.")
